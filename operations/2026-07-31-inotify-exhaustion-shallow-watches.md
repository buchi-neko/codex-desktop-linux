...
- [x] **実測完了**: `ai-news-collector` のタスクプレビューを実際に発生させて効果を確認（下記）
- [x] 検証スクリプトを正常系・異常系の両方でテスト（異常系で誤OKを出すバグを1件発見・修正）

## 実測結果（プレビュー操作後）

| 指標 | 対策前 | 対策後 |
|---|---|---|
| `git-repo-watcher` の起動 | — | **7回**（プレビューは確かに発生した） |
| `ENOSPC` | 614件 | **0件**（総数614のまま増えず） |
| Codex関連の watch | 62,664 | **22**（約2,850分の1） |
| 使用率 | 95.6% | **0.008%**（22 / 262,144） |

監視自体は7回張られているのに watch が22個しか増えていない ＝ 再帰監視が非再帰へ
落ちている動かぬ証拠。対策は意図どおり機能している。
